...
type TestTable struct {
	ID   mssql.UniqueIdentifier `boil:"Id" json:"Id" toml:"Id" yaml:"Id"`
...
type whereHelpermssql_UniqueIdentifier struct{ field string }

func (w whereHelpermssql_UniqueIdentifier) EQ(x mssql.UniqueIdentifier) qm.QueryMod {
	return qmhelper.Where(w.field, qmhelper.EQ, x)
}
func (w whereHelpermssql_UniqueIdentifier) NEQ(x mssql.UniqueIdentifier) qm.QueryMod {
	return qmhelper.Where(w.field, qmhelper.NEQ, x)
}
func (w whereHelpermssql_UniqueIdentifier) LT(x mssql.UniqueIdentifier) qm.QueryMod {
	return qmhelper.Where(w.field, qmhelper.LT, x)
}
func (w whereHelpermssql_UniqueIdentifier) LTE(x mssql.UniqueIdentifier) qm.QueryMod {
	return qmhelper.Where(w.field, qmhelper.LTE, x)
}
func (w whereHelpermssql_UniqueIdentifier) GT(x mssql.UniqueIdentifier) qm.QueryMod {
	return qmhelper.Where(w.field, qmhelper.GT, x)
}
func (w whereHelpermssql_UniqueIdentifier) GTE(x mssql.UniqueIdentifier) qm.QueryMod {
...
var TestTableWhere = struct {
	ID   whereHelpermssql_UniqueIdentifier
	Date whereHelpertime_Time
}{
	ID:   whereHelpermssql_UniqueIdentifier{field: "[dbo].[TestTable].[Id]"},
	Date: whereHelpertime_Time{field: "[dbo].[TestTable].[Date]"},
}
...
var (
	testTableAllColumns            = []string{"Id", "Date"}
	testTableColumnsWithoutDefault = []string{"Id", "Date"}
	testTableColumnsWithDefault    = []string{}
	testTablePrimaryKeyColumns     = []string{"Id"}
	testTableGeneratedColumns      = []string{}
)
...
func (q testTableQuery) One(ctx context.Context, exec boil.ContextExecutor) (*TestTable, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: failed to execute a one query for TestTable")
	}
...
func (q testTableQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to count TestTable rows")
	}
...
func (q testTableQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models: failed to check if TestTable exists")
	}
...
func TestTables(mods ...qm.QueryMod) testTableQuery {
	mods = append(mods, qm.From("[dbo].[TestTable]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[TestTable].*"})
	}

	return testTableQuery{q}
}

// FindTestTable retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindTestTable(ctx context.Context, exec boil.ContextExecutor, iD mssql.UniqueIdentifier, selectCols ...string) (*TestTable, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[TestTable] where [Id]=$1", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: unable to select from TestTable")
	}
...
func (o *TestTable) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models: no TestTable provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[TestTable] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[TestTable] %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to insert into TestTable")
	}
...
func (o *TestTable) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models: unable to update TestTable, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE [dbo].[TestTable] SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update TestTable row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by update for TestTable")
	}
...
func (q testTableQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all for TestTable")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected for TestTable")
	}
...
func (o TestTableSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[TestTable] SET %s WHERE %s",
...
func (o *TestTable) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no TestTable provided for upsert")
	}
	...
	if !cached {
		...
		for i, v := range insert {
			if strmangle.ContainsAny(testTablePrimaryKeyColumns, v) && strmangle.ContainsAny(testTableColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("models: unable to upsert TestTable, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(testTableAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("models: unable to upsert TestTable, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[TestTable]", testTablePrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(testTablePrimaryKeyColumns))
		copy(whitelist, testTablePrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(testTableType, testTableMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert TestTable")
	}
...
func (o *TestTable) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[TestTable] WHERE [Id]=$1"
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete from TestTable")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by delete for TestTable")
	}
...
func (q testTableQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from TestTable")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by deleteall for TestTable")
	}
...
func (o TestTableSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[TestTable] WHERE " +
...
func (o *TestTableSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[TestTable].* FROM [dbo].[TestTable] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, testTablePrimaryKeyColumns, len(*o))
...
func TestTableExists(ctx context.Context, exec boil.ContextExecutor, iD mssql.UniqueIdentifier) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[TestTable] where [Id]=$1) then 1 else 0 end"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if TestTable exists")
	}

	return exists, nil
}

// Exists checks if the TestTable row exists.
func (o *TestTable) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return TestTableExists(ctx, exec, o.ID)
}
